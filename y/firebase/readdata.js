...
import { getFirestore, updateDoc, doc, getDoc,  } from 'https://www.gstatic.com/firebasejs/9.8.2/firebase-firestore.js';
...
//   let data=document.querySelector('#mc').textContent
...
if (docSnap.exists()) {
  ...
  document.querySelector('#user_name').textContent=`Welcome ${name.name} 👋`
  }catch(e){}
  try{
 document.querySelector('#my_cart').textContent=`${name.name}'s Shopping Cart 😎`    
    }catch(e){}
